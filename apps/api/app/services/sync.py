...
class SyncService:
    ...
    def create_sync_run(self, user_context: dict, store_id, mode: str, idempotency_key: str | None) -> dict:
        sync_run = self.module.create_sync_run(user_context, store_id, mode, idempotency_key)
        if sync_run.pop("_enqueue_sync_run", False):
            self._enqueue_sync_run(sync_run["id"])
        return sync_run
    ...
    def retry_sync_run(self, user_context: dict, store_id, sync_run_id, idempotency_key: str | None) -> dict:
        sync_run = self.module.retry_sync_run(user_context, store_id, sync_run_id, idempotency_key)
        if sync_run.pop("_enqueue_sync_run", False):
            self._enqueue_sync_run(sync_run["id"])
        return sync_run

    def execute_sync_run(self, sync_run_id: str) -> None:
        self.module.execute_sync_run(sync_run_id)
    ...
    @staticmethod
    def _enqueue_sync_run(sync_run_id: str) -> None:
        from app.tasks.sync import run_store_sync

        run_store_sync.delay(sync_run_id)
