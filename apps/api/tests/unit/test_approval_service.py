...
def test_approve_delegates_to_module_and_enqueues_when_requested():
    db = Mock()
    module = Mock()
    module.approve.return_value = {"id": "approval-1", "status": "approved", "_enqueue_publish": True}
    service = ApprovalService(db=db, module=module)
    user_context = {"user": {"id": "u1"}, "organization": {"id": "o1"}}

    with patch.object(service, "_enqueue_publish") as enqueue_mock:
...
def test_retry_execution_delegates_to_module_and_enqueues_when_requested():
    db = Mock()
    module = Mock()
    module.retry_execution.return_value = {"id": "approval-2", "status": "approved", "_enqueue_publish": True}
    service = ApprovalService(db=db, module=module)
    user_context = {"user": {"id": "u1"}, "organization": {"id": "o1"}}
